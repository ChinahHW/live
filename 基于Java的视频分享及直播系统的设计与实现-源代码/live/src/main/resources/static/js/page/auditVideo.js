...
function tableInit(data) {
	...
	for(var i = 0; i < data.length; i++) {
		var btn = '<button data-id="' + data[i].id + '" class="layui-btn layui-btn-xs data-pass vip">通过</button><button data-id="' + data[i].id + '" class="layui-btn layui-btn-xs layui-btn-danger data-delete vip">不通过</button>';
		html += '<tr>' +
			'        <td>' + data[i].id + '</td>' +
			'        <td>' + data[i].name + '</td>' +
			'        <td>' + data[i].content + '</td>' +
			'        <td>' + data[i].url + '</td>' +
			'        <td><img src=../img/' + data[i].videopicture + ' /></td>' +
			'        <td>' + btn + '</td>' +
			'    </tr>'
	}
...
function tableEvent() {
	...
	$(".data-pass").unbind();
	$(".data-pass").bind("click", function() {
		var id = $(this).attr("data-id");
		var obj = {};
		obj.id = id;
		obj.isalive = 1;
		layer.confirm('您确定要通过这条数据吗?', {
			btn: ['确定', '取消'] //按钮
		}, function() {
			videoPass(obj);
		}, function() {
...
function videoPass(obj) {
	layer.load(2);
	$.ajax({
		url: urlconfig.videoUpdate,
		data: obj,
		async: true,
		success: function(data) {
			layer.closeAll();
