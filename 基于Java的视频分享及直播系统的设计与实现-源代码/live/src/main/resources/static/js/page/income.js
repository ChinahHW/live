...
$.ajax({
	...
	success: function(data) {
		...
		$(".incomeAttenProjectId").html(html);
	},
	error: function(err) {
		console.log(err)
	}
...
$.ajax({
	...
	success: function(data) {
		...
		for(var i = 0; i < data.data.length; i++) {
			html +=
				'        <option value="' + data.data[i].incomeTypeName + '">' + data.data[i].incomeTypeName + '</option>'
		}
		$("#incomeTypeCode").html(html);
		$("#incomeType").html(html);
		$("#incomeType1").html(html);
	},
	error: function(err) {
		console.log(err)
	}
...
$("#codeSearch").bind("click", function() {
	var incomeCode = $("#code").val();
	var incomeCode1 = $("#incomeTypeCode").val();
	var incomeCode2 = $("#incomeStateCode").val();
	search = {};
	if(incomeCode || incomeCode1 || incomeCode2) {
		search.projectName = incomeCode;
		search.incomeType = incomeCode1;
		search.incomeState = incomeCode2;
	}
	console.log(search);
	search.currentPage = 1;
	search.pageSize = 10;
...
function layerInit(current, total) {
	layui.use(['form', 'layedit', 'laydate', 'laypage', 'table'], function() {
		...
		//日期
		laydate.render({
			elem: '#incomeCreateDateMin'
		});
		laydate.render({
			elem: '#incomeCreateDateMax'
		});
		laypage.render({
...
function tableInit(data) {
	...
	for(var i = 0; i < data.length; i++) {
		var btn = '<button data-id="' + data[i].incomeId + '" class="layui-btn layui-btn-xs data-update vip">修改</button><button data-id="' + data[i].incomeId + '" class="layui-btn layui-btn-xs layui-btn-danger data-delete vip">删除</button>';
		html += '<tr>' +
			/* '        <td>'+data[i].incomeCode+'</td>' +*/
			'        <td>' + data[i].incomeSerialCode + '</td>' +
			'        <td>' + data[i].incomeCertificateCode + '</td>' +
			'        <td>' + data[i].incomeType + '</td>' +
			'        <td>' + data[i].projectName + '</td>' +
			'        <td>' + data[i].incomeState + '</td>' +
			'        <td>' + (data[i].incomeMoney ? data[i].incomeMoney : '') + '</td>' +
			'        <td>' + data[i].incomeDes + '</td>' +
			'        <td>' + btn + '</td>' +
			'    </tr>'
	}
	$("#tableList").html(html);
...
function tableEvent() {
	...
	$(".data-update").bind("click", function() {
		...
		for(var i = 0; i < dataList.length; i++) {
			if(id == dataList[i].incomeId) {
				var data = dataList[i];
				console.log(data);
				for(var name in data) {
					$("#" + name).val(data[name]);
				}
				$("input[value='" + data.incomeState + "']").prop('checked', 'true');
				form.render();
				break;
			}
		}
